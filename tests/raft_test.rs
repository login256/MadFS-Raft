...
async fn generic_test(
    ...
    let ck = t.make_client(&t.all()).await;
...
async fn unreliable_one_key_3a() {
    ...
    let all = t.all();
    let ck = t.make_client(&all).await;
...
async fn one_partition_3a() {
    ...
    let (p1, p2) = t.make_partition().await;
    t.partition(&p1, &p2);

    // connect ckp1 to p1
    let ckp1 = t.make_client(&p1).await;
    // connect ckp2a to p2
    let ckp2a = t.make_client(&p2).await;
    let ckp2a_name = ckp2a.id();
    // connect ckp2b to p2
    let ckp2b = t.make_client(&p2).await;
    ...
    t.connect_all();
    t.connect_client(ckp2a_name, &all).await;
    t.connect_client(ckp2b_name, &all).await;
...
async fn snapshot_rpc_3b() {
    ...
    t.partition(&[0, 1], &[2]);
    {
        let ck1 = t.make_client(&[0, 1]).await;
        for i in 0..50 {
    ...
    t.partition(&[0, 2], &[1]);
    {
        let ck1 = t.make_client(&[0, 2]).await;
        ck1.put("c", "C").await;
        ck1.put("d", "D").await;
        ck1.check("a", "A").await;
        ck1.check("b", "B").await;
